...
class EncoderLayer(nn.Module):
    def __init__(self, args, depth, is_moe_layer=False, is_encoder_decoder=False):
        ...
        if args.ffn_adapt:
            if args.ffn_mode == 'text':
                self.adaptmlp_text = Adapter(self.args, dropout=0.1, bottleneck=args.ffn_num,
            ...
            elif args.ffn_mode == 'vision':
                self.adaptmlp_vision = Adapter(self.args, dropout=0.1, bottleneck=args.ffn_num,
                                        init_option=args.ffn_adapter_init_option,
                                        adapter_scalar=args.ffn_adapter_scalar,
                                        adapter_layernorm_option=args.ffn_adapter_layernorm_option,
                                        )
            elif args.ffn_mode == 'both':
                self.adaptmlp_text = Adapter(self.args, dropout=0.1, bottleneck=args.ffn_num,
                                    init_option=args.ffn_adapter_init_option,
                                    adapter_scalar=args.ffn_adapter_scalar,
                                    adapter_layernorm_option=args.ffn_adapter_layernorm_option,
        ...
        if args.ffn_enhance:
            # self.enhancemlp = MultiwayWrapper(
            #     args,
            #     self.build_ffn(
            #         self.embed_dim,
            #         self.args,
            #     ),
            # )
            self.enhancemlp = self.build_ffn(
                    self.embed_dim,
                    self.args,
                )
    ...
    def forward(self, x, encoder_padding_mask, attn_mask=None, rel_pos=None, multiway_split_position=None, incremental_state=None):
        ...
        # using adapter here
        if self.ffn.split_position == -1 and self.args.ffn_adapt and self.args.ffn_mode in ['text', 'both']:
            adapt_x  =  self.adaptmlp_text(x)
        elif self.ffn.split_position == 0 and self.args.ffn_adapt and self.args.ffn_mode in ['vision', 'both']:
            adapt_x  = self.adaptmlp_vision(x)
        # elif self.args.ffn_mode == 'both':
        #     x1, x2 = torch.split(
        #         x,
        #         [self.ffn.split_position, x.size(self.ffn.dim) - self.ffn.split_position],
        #         dim=self.ffn.dim,
        #     )
        #
        #     y1, y2 = self.adaptmlp_text(x1), self.adaptmlp_vision(x2)
        #     adapt_x  =  torch.cat([y1, y2], dim=self.dim)

        # if self.args.ffn_adapt and self.args.ffn_option == 'parallel':
        ...
        # inter result here
        if self.args.ffn_adapt:
            try:
                if self.args.ffn_option == 'sequential':
                    x = self.adaptmlp(x)
                elif self.args.ffn_option == 'parallel':
                    # optional
                    # x = self.residual_connection(x, residual)
                    x = x + adapt_x
                else:
