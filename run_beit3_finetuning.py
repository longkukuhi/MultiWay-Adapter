...
def get_args():
    ...
    parser.add_argument('--ffn_adapter_init_option', type=str, default='lora',
                        help=')')
    parser.add_argument('--ffn_enhance', default=False, action='store_true', help='whether activate AdaptFormer')
    parser.add_argument('--ffn_mode', default="both", type=str,)

    parser.add_argument('--ffn_adapter_scalar', default="0.1", type=str,
                        help='ffn_adapter_scalar')
    parser.add_argument('--ffn_adapter_layernorm_option', default="none", type=str,
                        help='ffn_adapter_layernorm_option')
    parser.add_argument('--ffn_option', default="parallel", type=str,
                        help='ffn_option')
    parser.add_argument('--d_model', default=768,type=int, help='d_model')
    parser.add_argument('--fulltune', default=False, action='store_true', help='full finetune model')
    parser.add_argument('--loss', default='both', type=str, help='ffn_adapter_init')
...
def main(args, ds_init):
    ...
    if args.eval:
        exp_tag = f"eval_{args.task}_{args.seed}seed_{args.input_size}imageres_{args.finetune.split('/')[-1]}/"
    else:
        exp_tag = f"{args.model}_{args.ffn_mode}mode_{args.ffn_num}ffn_num_{args.ffn_adapter_scalar}raito_{args.ffn_enhance}enheancemlp_{args.seed}seed_{args.epochs}epochs_{args.task}tasks_{args.finetune.split('/')[-1]}/"
    ...
    model = create_model(
        ...
        ffn_adapter_layernorm_option=args.ffn_adapter_layernorm_option,
        ffn_adapter_init_option=args.ffn_adapter_init_option,
        ffn_adapter_scalar=args.ffn_adapter_scalar,
        ffn_num=args.ffn_num,
    ...
    if args.finetune:
        missing_key = utils.load_model_and_may_interpolate(args.finetune, model, args.model_key, args.model_prefix)
    else:
        missing_key = None

    # free all but adapter
    if args.ffn_adapt and not args.fulltune and not args.eval:
